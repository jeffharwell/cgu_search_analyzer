...
sys.path.append("./all_query_data")
...
class Expander():
    ...
    def connect(self):
        ...
        """
        ##self.connection = pyodbc.connect('DRIVER={SQL Server};Trusted_Connection=yes;SERVER=WIN-20IR78KE8UV\SQLSERVER2012;DATABASE=GoogleCorpus;UID=Administrator')
...
#        self.connection2 = pyodbc.connect('DRIVER={SQL Server};SERVER=134.173.236.21;DATABASE=umlsSmall;UID=sa;PWD=1SISATcguisl')
...
def main():
    ...
    data=[]
    filename="./%s/sampled2" % datadirectory # File to be read
    print "begin"
    ...
    for term in expander.terms: 
        c+=1
        print "%d / %d" % (c,l)
        terms = a.term_to_list(term)
        if len(terms) == number_of_grams:
            corpus_results = len(a.fetch_corpus_expansions(expander.cursor,terms,number_of_grams))
            if (corpus_results > 0):
                last_term = term
